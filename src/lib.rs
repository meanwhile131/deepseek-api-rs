        ...
        if is_new_object {
                ...
                && v.get("response").is_some()
            {
                ...
                if !self.initial_thinking_yielded
                    && let Some(thinking_val) = v.get("response").and_then(|resp| resp.get("thinking_content"))
                        && let Some(thinking_str) = thinking_val.as_str()
                ...
                if !self.initial_content_yielded && !self.initial_thinking_yielded
                    && let Some(content_val) = v.get("response").and_then(|resp| resp.get("content"))
